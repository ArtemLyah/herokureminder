...
async def back(query, reply_text="Choose a button"):
    ...
    db_tasks.set_user_settings(query.from_user.id, {"is_create_task":0})
...
async def show_task(query:types.CallbackQuery):
    tasks = db_tasks.get_task(query.from_user.id)
    if tasks:
...
async def create_task(query:types.CallbackQuery):
    db_tasks.set_user_settings(query.from_user.id, {"is_create_task":1})
...
async def callback_query_task(query:types.CallbackQuery):
    ...
    if re.match(r"delete \d+", query.data):
        ...
        db_tasks.remove_task(query.from_user.id, task_id)
...
async def show_menu(message:types.Message, reply_text="Choose a button", edit=False):
    ...
    db_tasks.set_user_settings(message.from_user.id, {"is_create_task":0})
    if edit:
...
async def other(message:types.Message):
    if db_tasks.get_from_user_settings(message.from_user.id, ["is_create_task"]):
        text = message.text
        ...
        if db_tasks.get_task(message.from_user.id, text):
            await message.answer("You already have the task")
            await show_menu(message)
        else:
            db_tasks.create_task(message.from_user.id, text, alarm_time)
